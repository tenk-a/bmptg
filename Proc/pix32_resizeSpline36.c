			...
			if (scaleType == 0) {	// (rscaleX != 1.0 && rscaleY != 1.0)
				for (y = y1; y <= y2; ++y) {
					for (x = x1; x <= x2; ++x) {
					 ...
					 #else	//実験: http://koujinz.cocolog-nifty.com/blog/2009/05/post-d4b2.html を参考.
					 		// X Y ごとでなく直線距離でweightを求める場合. (シャープネスが効いたような画になる)
